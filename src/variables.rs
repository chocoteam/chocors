...
use crate::{CHOCO_BACKEND, Sealed};
...
pub unsafe trait Variable<'model>: ModelObject<'model> + Sealed {
    fn name(&self) -> String {
        ...
        unsafe {
            let name_ptr = backend.lib.Java_org_chocosolver_capi_VariableApi_getName(backend.thread,self.get_raw_handle());
    ...
    fn is_instantiated(&self) -> bool {
        ...
        // - Guaranteed by caller as in trait safety comment
        unsafe {
            let result = backend.lib.Java_org_chocosolver_capi_VariableApi_isInstantiated(backend.thread, self.get_raw_handle());
            result != 0
        })
    }
    fn is_view(&self) -> bool {
        ...
        // - Guaranteed by caller as in trait safety comment
        unsafe {
            let result = backend.lib.Java_org_chocosolver_capi_VariableApi_isView(backend.thread, self.get_raw_handle());
            result != 0
        })
    }
}
